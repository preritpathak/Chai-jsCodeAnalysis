...
public class DataTransmissionSelectorDialog extends DialogWrapper {
    ...
    public interface Callback {
        void onOk(JsDocAnnotationHolder filledAnnotation);
    ...
    protected void doOKAction() {
        ...
        if (noDataTransmissionCheckBox.isSelected()) {
            JsDocAnnotationHolder filledAnnotation =
                    CodeInspectionUtil.createEmptyAnnotationHolderByType(ChaiAnnotationDataType.NotPersonalDataTransmission);
            super.doOKAction();
        ...
        JsDocAnnotationHolder filledAnnotation = CodeInspectionUtil.createEmptyAnnotationHolderByType(ChaiAnnotationDataType.DataTransmission);
    ...
    public DataTransmissionSelectorDialog(@Nullable Project project,
                                          JsDocAnnotationHolder preFilledAnnotation, Callback callback) {
        super(project, false);
...
//            PersonalDataGroup[] dataGroups = CodeInspectionUtil.getDataTypesByAccessIds(new String[]{id}, project);
            model.addElement(new JCheckBox(String.format("%s (%s)", id,
                    Arrays.stream(personalDataGroups).map(PersonalDataGroup::name).sorted().collect(Collectors.joining(", ")))));
        }
...
//            }
    ...
    static class PurposePanel extends JPanel {
        ...
        public PurposePanel(boolean initialEnabled) {
            ...
            appFunctionality = new JCheckBox(CollectionAttribute.ForAppFunctionality.split("\\.")[1]);
            appFunctionality.setEnabled(initialEnabled);
            add(appFunctionality);
            analytics = new JCheckBox(CollectionAttribute.ForAnalytics.split("\\.")[1]);
            analytics.setEnabled(initialEnabled);
            add(analytics);
            accountManagement = new JCheckBox(CollectionAttribute.ForAccountManagement.split("\\.")[1]);
            accountManagement.setEnabled(initialEnabled);
            add(accountManagement);
            advertisingOrMarketing = new JCheckBox(CollectionAttribute.ForAdvertisingOrMarketing.split("\\.")[1]);
            advertisingOrMarketing.setEnabled(initialEnabled);
            add(advertisingOrMarketing);
            developerCommunications = new JCheckBox(CollectionAttribute.ForDeveloperCommunications.split("\\.")[1]);
            developerCommunications.setEnabled(initialEnabled);
            add(developerCommunications);
            fraudPreventionAndSecurityAndCompliance = new JCheckBox((CollectionAttribute.ForFraudPreventionAndSecurityAndCompliance.split("\\.")[1]));
            fraudPreventionAndSecurityAndCompliance.setEnabled(initialEnabled);
            add(fraudPreventionAndSecurityAndCompliance);
            personalization = new JCheckBox(CollectionAttribute.ForPersonalization.split("\\.")[1]);
